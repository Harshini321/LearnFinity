...
def getSlots(insti_id):
    ...
    for slot in results:
        ...
        slots.append(x)
    return {"slots": slots, "status_code": 200, "message": "Slots fetched successfully"}

def postSlot(insti_id, slot_name):
    r = schedule.Slot.query.filter_by(slot_name = slot_name, insti_id = insti_id).first()
    if r:
        return {"message": "Slot already exists", "status_code": 400}
    else:
        obj = schedule.Slot(slot_name = slot_name, insti_id = insti_id)
        db.session.add(obj)
        db.session.commit()
        return {"id": obj.slot_id, "name": obj.slot_name, "insti_id": obj.insti_id, "status_code": 200, "message": "Slot added successfully"}

def deleteSlot(slot_id):
    obj = schedule.Slot.query.filter_by(slot_id = slot_id).first()
    db.session.delete(obj)
    db.session.commit()
    return {"id": obj.slot_id, "name": obj.slot_name, "insti_id": slot.insti_id, "status_code": 200, "message": "Slot deleted successfully"}
...
def deleteEntry(id):
    ...
    db.session.delete(obj)
    db.session.commit()
    return {"id": obj.entry_id, "day": obj.entry_day, "start_time": str(obj.entry_start_time), "end_time": str(obj.entry_end_time), "insti_id": obj.entry_insti_id, "status_code": 200, "message": "Entry deleted successfully"}

def slotEntry(slot_id, entry_id):
    r = schedule.Slot_Entry.query.filter_by(slot = slot_id, entry = entry_id).first()
    if r:
        return {"message": "Mapping already exists", "status_code": 400}
    else: 
        obj = schedule.Slot_Entry(slot = slot_id, entry= entry_id)
        db.session.add(obj)
        db.session.commit()
        return {"slot_id": obj.slot, "entry_id": obj.entry, "mapping_id": obj.slot_entry_id, "status_code": 200, "message": "Mapping added successfully"}
